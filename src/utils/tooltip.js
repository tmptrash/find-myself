...
// The bubble is clamped to screen edges so it never extends beyond the viewport.
//
function onDraw(inst) {
  if (inst.opacity <= 0.01 || !inst.activeTarget) return
  const { k } = inst
  const target = inst.activeTarget
  const pointerX = getTargetX(target)
  const pointerBaseY = getTargetY(target) + (target.offsetY ?? TOOLTIP_Y_OFFSET)
  const { width: textW, height: textH } = measureText(k, target.text, inst.font)
  const bubbleW = textW + BUBBLE_PADDING_X * 2
  const bubbleH = textH + BUBBLE_PADDING_Y * 2
  ...
  //
  let bubbleX = pointerX - bubbleW / 2
  ...
  //
  // Clamp bubble vertically (top edge)
  //
  let bubbleY = pointerBaseY - bubbleH
  const minY = SCREEN_EDGE_MARGIN + BUBBLE_BORDER_WIDTH
  bubbleY = Math.max(minY, bubbleY)
  //
  // Clamp pointer tip so it doesn't go below screen bottom
  //
  const pointerTipY = Math.min(
    pointerBaseY + POINTER_HEIGHT,
  ...
  //
  // Draw triangle pointer (border + fill), keeping pointerX within bubble width
  //
  const clampedPointerX = Math.max(
    bubbleX + POINTER_WIDTH,
    Math.min(bubbleX + bubbleW - POINTER_WIDTH, pointerX)
  )
  const borderPts = [
    k.vec2(clampedPointerX - POINTER_WIDTH / 2 - BUBBLE_BORDER_WIDTH, bubbleY + bubbleH - 1),
    k.vec2(clampedPointerX + POINTER_WIDTH / 2 + BUBBLE_BORDER_WIDTH, bubbleY + bubbleH - 1),
    k.vec2(clampedPointerX, pointerTipY + BUBBLE_BORDER_WIDTH)
  ]
  k.drawTriangle({
    p1: borderPts[0],
    p2: borderPts[1],
  ...
  const fillPts = [
    k.vec2(clampedPointerX - POINTER_WIDTH / 2, bubbleY + bubbleH - 1),
    k.vec2(clampedPointerX + POINTER_WIDTH / 2, bubbleY + bubbleH - 1),
    k.vec2(clampedPointerX, pointerTipY)
  ]
  k.drawTriangle({
    p1: fillPts[0],
    p2: fillPts[1],
  ...
  k.drawText({
    ...
    size: FONT_SIZE,
    font: inst.font,
    pos: k.vec2(bubbleX + bubbleW / 2, bubbleY + bubbleH / 2),
    anchor: "center",
